...
>
> **读完能做到：** 正确找到 2000 Hz 对应的频率箱　·　先写清矩阵的轴方向再求和　·　用极端信号验证实现是否正确

<picture>
  <source media="(max-width: 640px)" srcset="figures/mobile/00-course-position-22.svg">
  <img src="figures/desktop/00-course-position-22.svg" alt="第 22 课属于第 5 组「用少量数字概括频谱」，本组共 3 课">
</picture>

假设我们想观察一段音乐中，较低的声音成分何时占优势。电脑可以把录音排成一张图：每一列是一个短时间片段，每一行代表一个由低到高的位置。这张图叫作**声谱图**（[第 15 课](../零基础版_11-15/15-短时傅里叶变换STFT-怎样同时看见频率与出现时间.md)讲过），这些高低位置表示声音每秒振动多少次，也就是**频率**。
...
课程项目《三首曲子，一个分类器》共 23 步，这是第 22 步。这个特征本身不难，难的是"2000 Hz 落在第几个频率箱"这一步——算错了不会报错，只会悄悄给出错误的数。

1. 实现 `band_energy_ratio(spec, sr, n_fft, split_hz=2000)`，返回逐帧的低频能量与高频能量之比。
2. 用第 13 课的 `freq_axis` 找分界位置，**不要**用 `int(split_hz / sr * n_fft)` 这类近似写法。
3. 手工验证一次：构造一个只有 500 Hz 的信号和一个只有 5000 Hz 的信号，比值应该分别趋近于很大和很小。
4. 把逐帧结果的均值加进 `features.csv`。

**做完应该有**：`band_energy_ratio` 函数，两个极端信号验证通过。

**自检**：纯 500 Hz 信号的比值应该在 100 以上，纯 5000 Hz 的应该在 0.01 以下。如果两者都接近 1，说明分界位置算错了，或者矩阵的频率轴取反了。

上一步：[第 21 课 · 想清楚再算：三个统计量各回答什么](21-频域特征怎样概括一帧声音-带能量比质心与带宽.md)　·　下一步：[第 23 课 · 合并全部特征，检验能不能分开](23-频谱质心与带宽怎么计算-声音频率的中心与扩散.md)　·　[项目全貌](../课程项目/README.md)
