...
| 19 MFCC 是什么 | 主线：`source_course/19 - MFCCs Explained Easily/Mel-Frequency Cepstral Coefficients Explained Easily.pdf`；NotebookLM 补充博客稿：`notebooklm博客/ダウンロード (13).md`（Markdown，不是 `.ipynb`） | 上一课回顾 → 倒谱（cepstrum）→ 谱与倒谱、频率与倒频率（quefrency）→ 提升（liftering）→ 声音怎样产生：声门激励 × 声道传递函数 → 取对数把乘法变成加法 → 共振峰承载音色 → 用 DCT 把两者分开 → 为什么取前 12—13 个系数 → MFCC 的用途与优缺点 | **已修复：**按 PDF 顺序重写，并用合成谱、真实语音和 DCT 实验把“分开”变成可核对数字 | `lesson19_cepstrum.py` 已验证取对数把乘法变加法、倒频率两端的能量分布、提升后的包络恢复、DCT 去相关和前 13 个系数的变化保留率 |
| 20 用 Python 取 MFCC | 主线：`source_course/20 - Extracting MFCCs with Python/Extracting MFCCs.ipynb`；NotebookLM 补充博客稿：`notebooklm博客/ダウンロード (15).md`（Markdown，不是 `.ipynb`） | 载入 debussy → `librosa.feature.mfcc(n_mfcc=13)` → 看 shape → 画图 → `librosa.feature.delta` 求一阶差分 → `order=2` 求二阶 → 各自画图 → `np.concatenate` 拼成 39 行 | 已写 | `lesson20_mfcc.py`：四步对齐（`n_mels` / `fmax` / `norm` / `htk` 全改对才 0.0000）、`power_to_db` 的 `top_db=80` 暗桩、扰动一帧证明 delta 沿时间轴（9 列宽变 8 列）、13 / 13 / 13 → 39 行、三种风格的最近中心法判对率 |
| 21 频域特征 | 主线：`source_course/21 - Frequency-Domain Audio Features/Frequency-domain audio features.pdf`；NotebookLM 补充博客稿：`notebooklm博客/ダウンロード (14).md`（Markdown，不是 `.ipynb`） | 上一课回顾 → 频域特征有哪些 → 提取流程（波形 → 分帧加窗 → 傅里叶变换 → 频谱 → 算特征）→ 数学约定 mt(n)、N → 频带能量比：低频段与高频段的能量之比，衡量低频有多主导 → 谱质心：频谱的「重心」，对应听感上的明亮度 → 带宽：围绕质心的展开程度 | **已写：**严格停在概念和受控合成频谱，不提前使用真实音乐；补清真实 Hz 频率轴，以及 $p=1$ / $p=2$ 两种带宽定义 | `lesson21_freq_features.py`：质心同为 1000 Hz 时，$p=2$ 带宽可为 288.7 / 500.0 Hz；带宽同为 500 Hz 时，质心可为 1000 / 2000 Hz，证明三个特征不能互换 |
| 22 手写频带能量比 | 可执行主线：`source_course/22 - Implementing Band Energy Ratio from Scratch with Python/Implementing band energy ratio from scratch with Python.ipynb`；补充博客稿：`notebooklm博客/ダウンロード (12).md`（Markdown，不是 `.ipynb`） | 载入 debussy / redhot / duke → FRAME_SIZE 2048、HOP_SIZE 512 → `librosa.stft` → 写 `calculate_split_frequency_bin` → 写 `band_energy_ratio` → 三段各算一次 → `frames_to_time` 换成秒 → 三条曲线画在一起 | **已写：**保留原顺序，并复现、定位、修正矩阵轴与格距两个错误 | `lesson22_band_energy_ratio.py`：正确分界第 186 格（2002.59 Hz）；把 1292 个时间帧误作频率格数会切到第 234 格（2519.38 Hz），三段中位 BER 偏高 1.83—5.97 dB；向量化与逐帧循环最大差 5.684e-14 |
| 23 谱质心与带宽 | 可执行主线：`source_course/23 - Spectral centroid and bandwidth/Spectral centroid and bandwidth.ipynb`；补充博客稿：`notebooklm博客/ダウンロード (11).md`（Markdown，不是 `.ipynb`） | 载入 debussy / redhot / duke → FRAME_SIZE 1024、HOP_LENGTH 512 → `librosa.feature.spectral_centroid` → 看 shape → `frames_to_time` → 三条质心曲线 → `librosa.feature.spectral_bandwidth` → 三条带宽曲线 | **已写：**保留先质心后带宽的顺序，再补手写对齐；明确第 21 课的 $p=1$ 与库默认 $p=2$ 不可混用 | `lesson23_centroid_bandwidth.py`：质心与库最大差 1.917e-05 Hz，$p=2$ 带宽最大差 1.156e-05 Hz；三段上 $p=2$ 中位数是 $p=1$ 的 1.30—1.54 倍 |
