...
**可复现：AE 必须和帧长一起记录。** 上面量到了，帧长从 256 换到 4096，AE 平均值涨三分之一。所以特征表里存的不能只是 `ae_mean = 0.1631`，还得记着它是用哪组帧长帧移算出来的。这三个参数同样进[第 04 课](04-模拟声到数字录音-连续的声音怎样变成一串数字.md)那个 `config.py`，后面两课还会往这张表上加。
