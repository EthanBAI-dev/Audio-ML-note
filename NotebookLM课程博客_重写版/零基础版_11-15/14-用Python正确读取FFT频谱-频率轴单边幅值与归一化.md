...
录音每秒保存多少个数字，叫**采样率**。假设采样率为 22050 Hz，FFT 长度为 2048。数组中的第 41 个位置不是“41 Hz”，它对应

$$
41\times\frac{22050}{2048}\approx441.4\ \text{Hz}。
$$
...
从一段录音中直接截取 $N$ 个样本，左右边缘通常接不上。突然切断会让一个频率的能量散到邻近频率格，这种现象叫**频谱泄漏**。先用一个中间高、两端低的平滑形状逐点压低声音，叫**加窗**；常用的平滑形状之一叫 **Hann 窗**。
