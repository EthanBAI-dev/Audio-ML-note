...
## 动手做：第 13 步 · 把数组位置换算成 Hz

课程项目《三首曲子，一个分类器》共 23 步，这是第 13 步。真正做 FFT 之前，先解决一个最容易错、错了又不报错的问题：第 k 个数到底对应多少赫兹。

1. 在 `soundlab/spectral.py` 里实现 `freq_axis(n_fft, sr)`，返回单边频谱每个位置对应的频率。
2. 手算验证：`n_fft=1024`、`sr=22050` 时，第 0、1、100、512 个位置分别是多少 Hz？
3. 把你的结果和 `np.fft.rfftfreq(1024, 1/22050)` 逐个比对。
4. 在注释里写清楚：为什么单边频谱的长度是 `n_fft/2 + 1` 而不是 `n_fft/2`。

**做完应该有**：`freq_axis` 函数，与 numpy 完全一致。

**自检**：第 512 个位置应该正好是 11025 Hz，也就是采样率的一半。如果你的最后一个值不是 11025，多半是用 `linspace` 猜了终点——这是本课最常见的错误。

上一步：[第 12 课 · 把匹配实验改成复数版](12-傅里叶变换为什么使用复数-同时测出频率强度与起始位置.md)　·　下一步：[第 14 课 · 正确读一次频谱](14-用Python正确读取FFT频谱-频率轴单边幅值与归一化.md)　·　[项目全貌](../课程项目/README.md)
