...
>
> **读完能做到：** 写出 RMS 与过零率的实现　·　解释 RMS 为什么比最高峰更稳　·　用两条曲线联合区分语音和噪声

<picture>
  <source media="(max-width: 640px)" srcset="figures/mobile/00-course-position-09.svg">
  <img src="figures/desktop/00-course-position-09.svg" alt="第 9 课属于第 2 组「把录音变成可计算片段」，本组共 5 课">
</picture>
...
$$
\operatorname{RMS}=\sqrt{\frac{1}{K}\sum_{k=0}^{K-1}x_k^2}
$$
...
## 动手做：第 9 步 · 实现 RMS 与过零率，写出第一版特征表

课程项目《三首曲子，一个分类器》共 23 步，这是第 9 步。这一步之后，`features.csv` 第一次有了内容——三个纯时域特征。

1. 实现 `rms(y, ...)` 和 `zero_crossing_rate(y, ...)`，都返回逐帧序列。
2. 对 `dataset.csv` 里每个片段，算出三种特征的**均值和标准差**，共 6 个数字。
3. 写出 `features.csv`：每行一个片段，列为 `ae_mean, ae_std, rms_mean, rms_std, zcr_mean, zcr_std, label`。
4. 画一张散点图：横轴 `zcr_mean`，纵轴 `rms_std`，三类用三种颜色。

**做完应该有**：`features.csv` 的前 6 列，以及那张散点图。

**自检**：散点图上三类应该已经出现部分分离，尤其摇滚会聚在右侧（过零率高）。完全重合说明特征算错了或者标签串行了——用 `groupby(label).mean()` 核对一下每类的均值。

上一步：[第 08 课 · 实现振幅包络](08-振幅包络怎么计算-从逐帧最大值到可靠时间轴.md)　·　下一步：[第 10 课 · 手写一次频率试探](10-傅里叶变换的直觉-电脑怎样从复杂波形里找出频率.md)　·　[项目全貌](../课程项目/README.md)
