...
>
> **读完能做到：** 写出边界清楚的振幅包络实现　·　处理尾帧并让时间轴对齐波形　·　说出帧长怎样改变包络的细节
...
$$
\operatorname{AE}(m)=\max_{0\le k<K}\left|x(mH+k)\right|
$$
...
课程项目《三首曲子，一个分类器》共 23 步，这是第 8 步。第一个自己写的特征。重点不在公式，而在边界：绝对值、尾帧、时间轴。

1. 在 `soundlab/time_features.py` 里实现 `amplitude_envelope(y, frame_length, hop_length)`，返回每帧的最大绝对值。
2. 同时返回每帧对应的时间（帧中心，单位秒），让曲线能和波形对齐画在一起。
3. 按第 08 课的讨论处理结尾：明确说明你是丢掉不足一帧的尾巴，还是补零，并在注释里写出理由。
4. 用同一段音频对比你的结果和 librosa 的分帧结果，确认帧数一致。

**做完应该有**：`amplitude_envelope`，以及一张包络叠在波形上的图。

**自检**：包络曲线必须完全贴着波形的外缘，不能穿进去也不能浮在外面。如果包络整体偏低，多半是忘了取绝对值；如果时间轴对不齐，检查你用的是帧起点还是帧中心。

上一步：[第 07 课 · 先看看三种时域特征分不分得开](07-振幅包络RMS与过零率-不用频谱能看出什么.md)　·　下一步：[第 09 课 · 实现 RMS 与过零率，写出第一版特征表](09-RMS与过零率怎么计算-用整体强弱和翻越零线次数检查声音.md)　·　[项目全貌](../课程项目/README.md)
