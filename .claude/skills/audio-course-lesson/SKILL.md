...
**职责边界：**通用课程开发规则的唯一版本位于 `~/.codex/skills/build-tutorial-course/SKILL.md`。新建课程、批量重构、素材扫描、课程脉络表和通用交接先读那份规则；本 skill 只补充音频课程的零基础术语、事实准确、音频实验、公式、配图和仓库约定。
...
**动手实验量出来的数字必须配图。** 那是这套结构最值钱的产出，只留文字太浪费。

**时间和频率是连续轴，画波形、曲线或谱线；只有横轴是真正的离散类别时才使用柱状图。**
...
**规则：每个概念只有一课完整讲授，别处只做理解当前内容所需的最短回顾。** 归属不能靠关键词次数判断，也不能在文章写完后再移动定义；它必须来自源 PPT / notebook 的课程顺序。
...
最重要的边界是：07 完整讲 AE、RMS、ZCR 的定义、公式和用途；08 / 09 按 notebook 实现和比较，不再重走定义课。10 建立傅里叶整体直觉；03 若为了音色展示谱线，只观察，不提前讲计算；01 完全不讲频谱和声谱图。
