...
const labels = [
  ['课程', '导论'], ['声音、波形', '与音高'], ['分贝、响度', '与音色'],
  ['模拟声', '怎样数字化'], ['五种', '特征分类'],
  ['两条特征', '提取流水线'], ['三种', '时域特征'], ['实现', '振幅包络'],
...
const fullLabels = [
  '课程导论', '声音、波形与音高', '分贝、响度与音色', '模拟声怎样数字化', '五种特征分类方法',
  '两条特征提取流水线', '三种时域特征', '实现振幅包络', '实现 RMS 与过零率', '傅里叶变换的直觉',
...
function mobile(lesson) {
  ...
  for (let k = 0; k < n; k += 1) {
    ...
    if (on) s += T(W - pad - 14, y + 29, '正在读', { size: 14, weight: 700, fill: color, anchor: 'end' });
  }

  const sy = y0 + n * (rowH + gap) + 6;
  s += stageStrip(pad, sy, W - pad * 2, si, { labelY: 22, size: 14 });
