...
另外保留原有仓库约定：`<picture>` 双版式配图，以及 `check-readability.mjs` / `check-markdown-math.mjs` 两道机检。

---

# 写法硬规矩（用户 2026-09-02 定的，优先级高于上面的结构）
...
- ✗ 摆成三种样子给程序看 / 摆错了姿势 / 三种摆法
- ✓ 用三种方式读取 / 读取方式选错了 / 三种读取方式

- ✗ 「摆错了姿势，关键证据在进入程序之前就没了」
- ✓ 「读取方式选错了，你要找的那个特征在进入程序之前就已经丢掉了」

## 生活化的例子必须连回主体

举例子、打比方是为了让人理解，但**举完必须有一句把它接回来**，让读者知道这个例子和原来那个主体是什么关系。

> 三份材料都真实，但「第 78 分钟那个进球」只在第一份和第三份里找得到。
> **换句话说：同一场比赛，记录方式决定了你事后还能查到什么。** 电脑读录音也是这样。

少了加粗那一句，读者只记住了足球，没记住录音。

## 导读要帮上理解，不是复述

判据很简单：**把导读盖住再读正文，理解成本有没有变高？** 没有变高，这段导读就是废话。

一段合格的导读要给出三样东西：这一课的**主语和谓语**、一个**读者认得出的现象**、以及**选错了会怎样**。

- ✗ 「同一段录音有三种整理方式，每一种都会丢掉另外两种留下的东西。」
- ✓ 「电脑读一段录音，可以用波形、频谱、声谱图三种方式。读取方式选错了，你要找的那个特征在进入程序之前就已经没了。」

## 每个数字后面必须跟一句「所以呢」

写出一个数字，读者不会自动知道它是大是小、是好是坏。**数字后面必须接一句它意味着什么**，否则读者看完等于没看。

- ✗ 「那声敲击只让整段频谱变了 0.17%，在声谱图上却是旁边位置的 11.4 倍。」
  ——0.17% 算多还是算少？11.4 倍又代表什么？读者没有参照。
- ✓ 「用整段频谱去读，那声敲击只让数值动了 0.17%，**小到和正常的噪声起伏分不开，程序不可能发现它响过**；换成声谱图去读，敲击那一瞬间的数值是旁边的 11.4 倍，**一眼就能指出它落在第几秒**。」

后果也一样，写出来还要再往下推一步：

- ✗ 「你要找的那个特征在进入程序之前就已经没了。」——没了会怎样？
- ✓ 「你要找的那个声音，程序就永远找不到——**换再厉害的程序也补不回来**。」

判据：正文里每出现一个数字、每出现一句「就会怎样」，都自问一次**「所以呢？」**。答不出下一句，这句话就还没写完。

## 标题同样要有主语和谓语

术语不要顶在最前面——读者看到的第一样东西不该是三个专业名词，尤其当正文第一段讲的是手机、音量条这种日常场景时。

- ✗ 波形、频谱与声谱图：电脑识别声音时该「看」什么？
- ✓ 电脑读录音的三种方式：波形、频谱与声谱图

一个系列里不要每篇都套同一个句式模子，六篇长得一样会显得像自动生成的。
