...
动手算之前先听：[德彪西](../../source_course/audio_resources/debussy.wav)、[摇滚](../../source_course/audio_resources/redhot.wav)、[艾灵顿公爵](../../source_course/audio_resources/duke.wav)。

听的时候只留意一件事：**低音那一头有多满。** 德彪西那段是钢琴独奏，低音区厚、高频只有一点点琴槌的声响；摇滚那段的镲片和失真吉他一直在上面顶着；艾灵顿那段介于两者之间，管乐和节奏组都占着中频。

先有这个印象，后面算出来的三条曲线才有东西可对照——**曲线排出的高低顺序如果和耳朵完全对不上，那多半是代码错了，不是耳朵错了。** 这一课末尾就会遇到这样一次。
...
还差一步才能画：`ber` 里有 1292 个数，它们的编号是帧号，不是秒。把帧号换成时间要用帧移：

```python
times = librosa.frames_to_time(np.arange(len(ber)), sr=22050, hop_length=512)
```

帧移是 512 个样本，所以相邻两帧相隔 $512 / 22050 \approx 23.2$ 毫秒，1292 帧铺满约 30 秒。**横轴一定要换成秒再画**，否则图上读出的“第 600”既可能是第 600 帧，也可能被误读成第 600 秒。
...
在这三段录音、这个 2000 Hz 分界下，德彪西低频侧相对最多，摇滚最少，艾灵顿在中间——**和开头听到的顺序一致**。这不是巧合，也不是什么了不起的验证，但它是最便宜的一道检查：算出来的数如果把摇滚排到低频最重的一端，那一定是哪里写错了。

但课程里每类只有一段音乐，**这只能描述三个样本，不能宣布整个音乐流派都有同样规律。**
